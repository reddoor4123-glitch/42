...
- `highest_remaining_trump()`, `lowest_remaining_in_suit()`, `best_remaining_card_for_suit()` were, at the time this summary was written, genuinely deferred — not blocked by any code dependency (they'd call `Domino.get_rank()`/`get_suit()` directly, which were never buggy), but held back until Fix 1 has been playtest-confirmed, as a confidence gate rather than a technical one. **Update (July 6, 2026): the confidence gate cleared.** `highest_remaining_trump()` and `best_remaining_card_for_suit()` are now implemented in `public_knowledge.gd` and consumed by `decide_play()` branch #11's generalized guaranteed-win check (see `Phase3_Objective_Audit.md`). `lowest_remaining_in_suit()` remains deferred — no consumer identified for it yet.
...
3. ~~Once Fix 1 has broader playtest coverage, revisit the deferred ranking-dependent queries (`highest_remaining_trump()`, `lowest_remaining_in_suit()`, `best_remaining_card_for_suit()`) — same delegation rule applies (must go through `Domino.get_rank()`/`get_suit()`, never re-derive comparison logic).~~ **Done July 6, 2026** for the first two (see the Finding above); `lowest_remaining_in_suit()` still has no identified consumer.
