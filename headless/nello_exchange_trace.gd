...
#
# Run:
#   "$GODOT" --headless --path . --script res://headless/nello_exchange_trace.gd
# ═══════════════════════════════════════════════════════════════════════════
...
func _init():
	...
	var f = FileAccess.open("res://headless/nello_exchange_trace_results.json", FileAccess.WRITE)
	f.store_string(JSON.stringify(output, "\t"))
	f.close()

	print("")
	print("Results written to res://headless/nello_exchange_trace_results.json")
