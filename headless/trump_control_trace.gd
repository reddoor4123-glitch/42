...
#
# Run:
#   "$GODOT" --headless --path . --script res://headless/trump_control_trace.gd > /dev/null 2>/tmp/stderr.txt
#   cat /tmp/stderr.txt   (should be empty)
#   then read res://headless/trump_control_trace_results.json
#
...
func _init():
	...
	var f = FileAccess.open("res://headless/trump_control_trace_results.json", FileAccess.WRITE)
	f.store_string(JSON.stringify(out, "\t"))
	f.close()

	print("")
	print("Results written to res://headless/trump_control_trace_results.json")
	quit(0)
