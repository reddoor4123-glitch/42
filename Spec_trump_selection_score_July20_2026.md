...
**Status:** ✓ Implemented and validated, July 20, 2026. See `AI_Bid_Behavior_Bug_Log.md`, Pattern D, and `headless/job6_results.json` for the validation run.
...
**6b. No-regression check on the 37 previously-correct hands.** Corrected formula: 38/40 correct vs. 37/40 for the current formula, zero regressions, one fix (the hand in 6a). Confirmed live in GDScript via `headless/job6_trump_selection_score_validation.gd`, reusing the 40 hands already generated by the Job 5 classification harness (`headless/job5_results.json`) rather than re-running the ~9-minute Monte Carlo simulation. **✓ Passed** — exactly one hand flips, and it's the expected one.
