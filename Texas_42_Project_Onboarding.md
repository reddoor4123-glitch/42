...
- `PublicKnowledge` is rebuilt fresh every time, never cached/mutated, so it can't drift from the state it represents. Six queries exist total (see §3); two are consumed today.
...
- Ranking-dependent queries beyond void tracking (`remaining_count()`, `count_remaining_trump()`) are implemented and available now that the ranking fix is playtest-confirmed, but have no consumer yet.
